...
#include "LineStandardInput.cginc"
#include "UnityPBSLighting.cginc"
#include "LineStandardUtils.cginc"
#include "LineStandardBRDF.cginc"
...
inline FragmentCommonData MetallicSetup (float2 i_tex)
{
...
inline FragmentCommonData FragmentSetup (float4 ni_tex, half3 i_eyeVec, half3 i_normalWorld, half3 i_viewDirForParallax, half3x3 i_tanToWorld, half3 i_posWorld)
{
	float2 i_tex = ni_tex.xy / ni_tex.zw; 
...
//-------------------------------------------------------------------------------------
half4 OutputForward (half4 output, half alphaFromSurface, float4 color)
{
...
half4 fragForwardBase (VertexOutputForwardBase i) : SV_Target
{
	...
	half occlusion = Occlusion(i.tex.xy / i.tex.zw);
	...
	c.rgb += Emission(i.tex.xy / i.tex.zw);

	UNITY_APPLY_FOG(i.fogCoord, c.rgb);
	return OutputForward (c, s.alpha, i.color);
}
...
)
{
	...
	#ifdef _EMISSION
		color += Emission (i.tex.xy / i.tex.zw);
	#endif
	...
	outDiffuse = half4(s.diffColor, occlusion) * i.color;
